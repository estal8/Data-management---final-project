...
The poverty database : <https://www.census.gov/data/tables/time-series/demo/income-poverty/historical-poverty-people.html>

The prison population database : <https://www.sentencingproject.org/research/>

The alcohol consumption database : <https://www.niaaa.nih.gov/publications/surveillance-reports> => <https://www.niaaa.nih.gov/sites/default/files/pcyr1970-2022.txt>
...
state_poverty |>
  summarise(
    ...
    nb_na_percentage_poor = sum(is.na(`Percent in poverty`))
  ) |>
  knitr::kable()
```
...
```{r}
new_names <- as.character(state_prisonpop[1, ]) # Column Year is named NA
...
#### Alcohol (potentially very difficult to clean)

```{r}
#| message: false

state_alcohol <- vroom(here("DATA_alcohol.csv"))
```
