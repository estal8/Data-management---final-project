...
```{r}
dict_states_abbrev <- dict_states |>
  mutate(state = substr(country, 4, 5)) |>
  select(state, State)
```
...
```{r}
police_shootings <- vroom("DATA_policeshootingto2024.csv")
```
```{r}
...
immigration <- vroom("DATA_stateimmigration.csv")
```
...
public_spending <- vroom("DATA_spending.csv")
```
...
state_income <- vroom("DATA_stateincome.csv")
