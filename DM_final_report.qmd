...
Despite these shared national trends, cross-state heterogeneity remains large throughout the entire period, as illustrated by the wide percentile bands. Evolution of violent crime and property crime in the US per states.
...
#### 2.1 Evolution of violent crime and property crime in the US per states.
...
#### 2.2 Evolution of violent crime and property crime among high-crime states
...
#### 3. Descriptive co-movement with economic structure
...
hausman_violent
```
#### Hypothesis : crime inertia ???

To look into crime persistence (i.e., the tendency of high-crime states to remain high-crime), we estimate a dynamic panel model including lagged crime rates as an explanatory variable.

```{r}
panel_reg <- panel_reg |>
  group_by(State) |>
  arrange(Year) |>
  mutate(crime_lag = lag(Data.Rates.Property.All, 1)) |>
  ungroup()
...
```{r}
model_gmm <- pgmm(
  Data.Rates.Property.All ~ lag(Data.Rates.Property.All, 1) + log_gdp + share_pretax_income_p90p100 | 
    lag(Data.Rates.Property.All, 2:4),
  data = panel_reg,
  index = c("State", "Year"),
  effect = "individual",
  model = "onestep"
...
#### 1. Descriptive overview of crime and new covariates
...
#### 2. Contemporary heterogeneity and structural correlates (2015–2018)
...
In contrast to the long-run analysis, the short-run period (2015–2018) offers limited time variation and a smaller effective sample size. As a result, the objective of this section is not to establish causal relationships, but to document conditional associations between crime and a small set of contemporaneous socio-economic variables that are frequently discussed in the recent literature.
...
panel_recent <- panel_full |>
  ...
  filter(!is.na(log_gdp),
         !is.na(share_pretax_income_p90p100))
```

```{r}
m2_property_base <- plm(
  Data.Rates.Property.All ~ log_gdp + share_pretax_income_p90p100,
  data   = panel_recent,
  index  = c("State", "Year"),
  model  = "within",
  effect = "twoways"   # state FE + year FE
)
```
...
```{r}
m2_property_police <- plm(
  Data.Rates.Property.All ~ log_gdp + share_pretax_income_p90p100 + nb_police_shootings,
  data   = panel_recent |> filter(!is.na(nb_police_shootings)),
  index  = c("State", "Year"),
  model  = "within",
  effect = "twoways"
)
...
```{r}
m2_property_imm <- plm(
  Data.Rates.Property.All ~ log_gdp + share_pretax_income_p90p100 + lpr_total,
  data   = panel_recent |> filter(!is.na(lpr_total)),
  index  = c("State", "Year"),
  model  = "within",
  effect = "twoways"
)
...
```{r}
m2_violent_police <- plm(
  Data.Rates.Violent.All ~ log_gdp + share_pretax_income_p90p100 + nb_police_shootings,
  data   = panel_recent |> filter(!is.na(nb_police_shootings)),
  index  = c("State", "Year"),
  model  = "within",
  effect = "twoways"
...
```{r}
m2_violent_imm <- plm(
  Data.Rates.Violent.All ~ log_gdp + share_pretax_income_p90p100 + lpr_total,
  data   = panel_recent |> filter(!is.na(lpr_total)),
  index  = c("State", "Year"),
  model  = "within",
  effect = "twoways"
