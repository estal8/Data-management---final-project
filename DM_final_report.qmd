...
panel_reg <- panel_full |>
  mutate(log_gdp = log(gdp)) # explain why this choice ?
```

We first look at the relationship between GDP and violent / property crime rates. Economic theory suggests that GDP should be a stronger predictor of property crime (primarily motivated by financial need) than of violent crime, which is driven by more complex social and psychological factors (Becker, 1968).

```{r}
model_violent <- plm(
  Data.Rates.Violent.All ~ log(gdp),
  data = panel_full |> filter(!is.na(gdp), !is.na(Data.Rates.Violent.All)),
  index = c("State", "Year"),
  model = "within"
)
...
```{r}
model_property <- plm(
  Data.Rates.Property.All ~ log(gdp),
  data = panel_full |> filter(!is.na(gdp), !is.na(Data.Rates.Property.All)),
  index = c("State", "Year"),
  model = "within"
)
...
As for violent crime, GDP alone fails to significantly predict violent crime rates (R² = 0.15, p > 0.05), indicating that violent offences are driven by factors beyond economic conditions.

Now, if we add inequalities : 
We measure income inequality using the share of pre-tax income accruing to the top 10% of the distribution, as provided by the World Inequality Database. A higher share indicates greater income concentration at the top, which could be associated with increased social tension and crime rates. **AJOUTER UNE RÉFÉRENCE**
