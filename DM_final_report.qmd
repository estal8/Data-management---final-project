...
model_ols <- lm(
  Data.Rates.Property.All ~ log_gdp + share_pretax_income_p90p100 + 
    unemployment_rate + prison_population + beer_per_capita_14plus,
  data = panel_reg
)
vif(model_ols)
```

All VIF values are below 5, the conventional threshold for concern, so our explanatory variables are sufficiently independent.
...
Now, we replace $log(GDP)$ by $Consumption$ to test another measure of economic conditions (Model 2).
...
We replace $Unemployment$ by $Poverty rate$ to test another measure of economic difficulties (Model 3).
...
model_property_4 <- plm(
  Data.Rates.Property.All ~ property_lag + violent_lag + 
    log_gdp + 
    share_pretax_income_p90p100 +
    unemployment_rate +
    prison_per_100k +
...
```{r}
model_violent_4 <- plm(
  Data.Rates.Violent.All ~ property_lag + violent_lag + 
    log_gdp + 
    share_pretax_income_p90p100 +
    unemployment_rate +
    prison_per_100k +
...
modelsummary(
  list(
    ...
    "Model 5: Two-way FE" = model_property_5
  ),
  vcov = robust_vcov,
...
modelsummary(
  list(
    ...
    "Model 5: Two-way FE" = model_violent_5
  ),
  vcov = robust_vcov,
  stars = c('*' = 0.05, '**' = 0.01, '***' = 0.001),
  coef_rename = c(
