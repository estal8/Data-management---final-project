...
The first question is whether state-level heterogeneity exists. We test this using two complementary approaches.The F-test compares the fixed effects model against pooled OLS. The null hypothesis is that all state fixed effects are jointly equal to zero ($H_0 = \alpha_{1} = … = \alpha_{2}$). We obtain $F=148.9$ with $p<0.001$.
...
These tests confirm that individual effects are present and statistically significant. Having established that, we still have to choose between random effects and fixed effects. To do so, we use the Hausman test : under $H_0$, both fixed effects and random effects are consistent, but random effects is more efficient; under $H_1$, only fixed effects is consistent.
...
Next, we test for autocorrelation and heteroscedasticity. 
...
**AFTER CONSUMPTION CHANGE => problem with panel full final !!! But I can't commit the changes**
