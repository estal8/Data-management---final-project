...
all_variables <- tibble(
  ...
  `Hypothesis / Rationale` = c(
    ...
    "Labour market slack (short-term economic stress)",
    "Persistent economic hardship (lower tail of distribution)"
  )
)
...
We include unemployment and poverty as controls rather than primary variables for two reasons. First, they capture dimensions of economic stress that are conceptually distinct from aggregate GDP: a state's economy may grow while unemployment rises or poverty persists, creating divergent effects on crime. Second, poverty reflects the lower tail of the income distribution, which may affect crime differently than top income concentration (inequality). 
By including these controls, we isolate the partial correlation between our variables of interest and crime, conditional on labour market conditions and economic hardship.
